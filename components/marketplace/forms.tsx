  ...
  return (
    <form
      onSubmit={(e) => {
        ...
        const form = e.currentTarget;
        const fd = new FormData(form);
        run(() => createProduct(fd), () => form.reset());
      }}
    >
      <MpCard className="p-6">
        <div className="grid gap-5 md:grid-cols-2">
          <div className="md:col-span-2">
            <Field label="Product title" required>
              <input name="title" required placeholder="Enter a clear, descriptive title" className={field} />
            </Field>
          </div>
          <div className="md:col-span-2">
            <Field label="Short description">
              <input name="summary" placeholder="A brief description shown in listings" className={field} />
            </Field>
          </div>
          ...
          <Field label="Category" hint={categories.length === 0 ? "No categories have been configured yet." : undefined}>
          ...
          <Field label="Price (USD)" hint="Set 0 for a free product.">
            <input name="price" type="number" min={0} step="0.01" defaultValue="0" className={field} />
          </Field>
          <Field label="URL slug" hint="Leave blank to generate from the title.">
            <input name="slug" placeholder="my-product" className={field} />
          </Field>
          <div className="md:col-span-2">
          ...
          <div className="md:col-span-2">
            <Field label="Full description">
              <textarea name="description" rows={7} placeholder="What the buyer receives, how to use it, and what is included." className={area} />
            </Field>
          </div>
          <div className="md:col-span-2 rounded-xl border border-line bg-bg-soft px-4 py-3.5 text-[12.5px] text-ink-soft">
            File upload becomes available once the Marketplace storage provider, maximum upload size
            and malware scanning are configured. Products save as drafts until then.
          </div>
        </div>
        ...
        <div className="mt-6 flex justify-end gap-2.5">
          <button type="reset" className="h-12 rounded-xl border border-line bg-white px-5 text-[13.5px] font-bold text-deep-navy hover:bg-bg-soft">
            Clear
          </button>
          <button
            type="submit"
